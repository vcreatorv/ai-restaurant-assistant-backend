...
func buildUserContent(in promptInput) string {
	...
	// Tail-reminder: дублирует три критичных правила формата прямо перед генерацией.
	...
	b.WriteString("\n\n--- Напоминание перед ответом (служебное) ---\n")
	b.WriteString("1. Названия блюд в тексте ВСЕГДА оборачивай в двойные звёздочки (Markdown bold): **Рибай**, **Ризотто с белыми грибами**.\n")
	b.WriteString("2. ID блюд (числа) НЕ пиши в тексте ответа — ни в скобках, ни через дефис, никак. ID нужен только в финальном JSON-блоке.\n")
	b.WriteString("3. В самом конце ответа ОБЯЗАТЕЛЬНО добавь блок ровно в такой обёртке (тройные backticks, не одинарные):\n")
	b.WriteString("```json\n")
